...
> - `PreTradeCheckAsync` — **implemented and wired**; `StrategyExecutionEngine` calls it when `UseAiPreTradeGate = True`. Result logged as `↳ AI: PROCEED` or `↳ AI: VETO`.
> - `IClaudeReviewService` interface — **implemented**; `ClaudeReviewService` is registered as Scoped. Mockable in tests via the interface.
> - `AnalyseBacktestResultsAsync` — **removed** (ARCH-17 removed the Maximum Effort / Backtest tab; this integration point no longer exists).
> - Post-trade AI explanation, session briefing, dynamic risk adjustment, feedback loop — **not yet implemented** as of this date. This document remains the design target for those features.
...
| `AnalyseBacktestResultsAsync` | ❌ Removed | Maximum Effort tab removed (ARCH-17) — this integration point no longer exists |
